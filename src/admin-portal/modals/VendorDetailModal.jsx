...
const VendorDetailModal = ({ vendor, onClose }) => {
  ...
  const fullName = [vendor.firstName, vendor.lastName].filter(Boolean).join(' ') || 'N/A';

  const addressParts = [
    [vendor.streetAddress, vendor.streetAddress2].filter(Boolean).join(', '),
    [vendor.city, vendor.region].filter(Boolean).join(', '),
    [vendor.postalCode, vendor.country].filter(Boolean).join(' '),
  ].filter(Boolean);
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {websiteHref
                ? <a href={websiteHref} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline break-all">{vendor.website}</a>
                : <p className="text-gray-400">N/A</p>}
            </div>
          </div>
          ...
          <div>
            ...
            {addressParts.length > 0
              ? <p className="text-gray-900 whitespace-pre-line">{addressParts.join('\n')}</p>
              : <p className="text-gray-400">N/A</p>}
          </div>
          ...
          {vendor.productsServices && (
            <div>
              ...
              <p className="text-gray-900 whitespace-pre-wrap">{vendor.productsServices}</p>
            </div>
          )}
          ...
          {vendor.companyInfo && (
            <div>
              ...
              <p className="text-gray-900 whitespace-pre-wrap">{vendor.companyInfo}</p>
            </div>
          )}
